...
def load_base_traces():
    """Loads the JSON event log and groups events into discrete traces."""
    ...
    # --- Append the Component to the Activity ---
    df['Activity'] = df['Activity'] + "_" + df['Component']
    
    # Extract vocabulary for anomaly injection (what activities belong to what components)
    ...
    # Group by CaseID to form traces (lists of event dictionaries)
    # We sort by Timestamp to ensure causal order is preserved
...
def inject_anomaly(trace, target_component, component_activities, random_seed=None):
    """
    Injects SEVERE generic logical anomalies into the target component's events.
    Every event belonging to the target component is subjected to heavy corruption.
    """
    rng = random.Random(random_seed)
    
    # Upgraded, highly destructive anomaly types
        ...
        if current_event.get('Component') == target_component:
            ...
            if anomaly_to_inject == "skipped":
                ...
                if len(anomalous_trace) > 0:
                    # Throw the event to a completely random earlier point in the trace
                ...
                # Replace with a completely random valid activity for this component
                ...
                # Create a severe data storm by duplicating the event 3 to 5 times
                storm_count = rng.randint(3, 5)
                for _ in range(storm_count):
                    anomalous_trace.append(dict(current_event))
        else:
            # Not the target component, pass through unchanged
            anomalous_trace.append(current_event)
            
    # Re-align the logical clock timestamps perfectly sequentially
...
def fragment_traces(traces, trace_fragment_length):
    """
    Chops traces into incremental prefixes for Online Behavior prediction.
    Example for window=5: Generates prefixes of length 5, 10, and Full.
    """
    fragmented_traces = []
    ...
    for trace in traces:
        ...
        for i in range(n_fragments):
            # Calculate the prefix length (e.g., 5, then 10, then 15)
            current_length = (i * trace_fragment_length) + trace_fragment_length
            fragment = trace[0:current_length]
            
            # Deep copy so we don't accidentally modify the original base trace
            frag_copy = [dict(event) for event in fragment]
            
            # Assign a unique CaseID to this specific fragment window
            for event in frag_copy:
                event['CaseID'] = f"{event['CaseID']}_W{current_length}"
                
            fragmented_traces.append(frag_copy)
            
        # Handle the remaining events (e.g., the final length 12)
        if remaining_events != 0:
            fragment = trace[0:trace_length] # Take the full trace
            frag_copy = [dict(event) for event in fragment]
...
def convert_to_event_log(traces_list):
    """Flattens a list of traces and converts them to a PM4Py XES Event Log."""
    if not traces_list:
        return None
    ...
    # PM4Py requires standard datetime timestamps.
...
def generate_datasets(training_test_percentage):
    ...
    # Online Behavior (Anomalous Traces)
...
def save_event_logs(ob_training_event_logs, ob_test_event_logs):
    """Exports all generated PM4Py objects into the required directory structure."""
